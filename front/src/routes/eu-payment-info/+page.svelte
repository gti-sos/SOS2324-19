<script>
	...
	async function DeleteAllInfo() {
		try {
			...
			if (status == 200) {
				let messageText = document.createElement('p');
				messageText.textContent = 'Borrando datos, la página se actualizará automáticamente. ';
				messageText.style.backgroundColor = 'pink';
				messageText.style.padding = '10px';
				messageText.style.position = 'fixed';
				messageText.style.top = '20px';
...
<ul>
	{#each payment as pepe}
		<li class="countryItem">
			...
			<button
				style="background-color: #0366d6; color: white; padding: 10px 20px; border: none; border-radius: 5px; cursor: pointer;"
				on:click={() => showCountryData(pepe.cci)}>MOSTRAR</button
			>
		</li>
	{/each}
</ul>
...
{#if showCountryDetails}
	<div>
		...
		<p>
			Actual Plan EU Amount Latest Adoption: {selectedCountryData.actual_plan_eu_amt_latest_adop}
		</p>
		...
		<p>
			EU Payment Rate Initial Plan EU Amount: {selectedCountryData.eu_payment_rate_init_plan_eu_amt}
		</p>
		<p>
			EU Payment Rate Actual Plan EU Amount: {selectedCountryData.eu_payment_rate_actual_plan_eu_amt}
		</p>
	</div>
{/if}
