<script>
	...
	function AFI1() {
		...
		dataproxyAFI.forEach((entry) => {
			...
			combinedData[country].millions_of_passenger_per_kilometres +=
				entry.millions_of_passenger_per_kilometres || 0;
		});
	...
	function AFI2() {
		...
		dataproxyAFI2.forEach((entry) => {
			...
			combinedData[country].cumulative_initial_pre_financing +=
				entry.cumulative_initial_pre_financing || 0;
		});
		countryData = Object.values(combinedData);
