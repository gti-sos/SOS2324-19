...
db_PHT.count({}, (error, count) => {
    if (error) {
        console.error("Error al contar los datos existentes en la base de datos:", error);
    } else {
        if (count === 0) {
            db_PHT.insert(initialData, (error, doc) => {
                if (error) {
                    console.error("Error al insertar datos iniciales en la base de datos:", error);
                } else {
                    console.log("Datos iniciales insertados correctamente en la base de datos.");
                    datos = initialData;
                }
            });
        } else {
            console.log("Ya existen datos en la base de datos. No se requiere inserción de datos iniciales.");
        }
    }
});
